...
export function CreateBilingual({}: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({});
    api
      .post("/bilinguals", data)
      .then((data) => {
        router.push(`/bilinguals/${data.data.data.id}/translations-edit`);
      })
      .catch((err) => {
        let errors;
        if (err.response?.data?.errors) {
          ...
          errors = { message: ["Произошла ошибка при создании билингвы"] };
        }
        setErrors(errors);
        setTimeout(() => {
