...
import { SearchBilinguals } from "../SearchBilinguals/SearchBilinguals";
import { FiltersBilinguals } from "../FiltersBilinguals/FiltersBilinguals";
import { useCallback, useEffect, useRef, useState } from "react";
import { usePathname, useSearchParams, useRouter } from "next/navigation";
import api from "@/app/api";
import styles from "./BilingualsIndex.module.css";
...
function getQueryFilters(filters: BilingualFilters, page: number): string {
  const filters_good = [];
...
export function BilingualsIndex({}: Props) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const isMounting = useRef<boolean>(true);

  const [filters, setFilters] = useState<BilingualFilters>({
  ...
  const [items, setItems] = useState<BilingualCardInterface[]>([]);
  const [paginate, setPaginate] = useState<PaginatePageInterface[]>([]);

  const createQueryString = useCallback(
    (filters: BilingualFilters, page: number) => {
  ...
  useEffect(() => {
    setLoading(true);
    api
      .get(`/bilinguals${getQueryFilters(filters, page)}`)
      .then((data) => {
    ...
    <div className="flex items-start justify-between gap-x-5">
      ...
      <div className="flex-auto">
        ...
        {!loading && items.length === 0 && (
          <div className="text-sm">Не нашлось билингв</div>
        )}
        {!loading && (
      ...
      <FiltersBilinguals filters={filters} setFilters={setFilters} />
    </div>
  );
}
