...
interface Props {
  ...
  setFilters: React.Dispatch<React.SetStateAction<LiteratureFilters>>;
}

export function Search({ filters, setFilters }: Props) {
  ...
  const [tags, setTags] = useState<LiteratureTagInterface[]>([]);
  const [searchedTags, setSearchedTags] = useState<LiteratureTagInterface[]>(
    []
  );
