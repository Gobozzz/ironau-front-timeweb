...
import { CourseCard } from "@/app/components/shared/Cards/CourseCard";

interface Props {
  className?: string;
}

export function MyCourses({ className = "" }: Props) {
  ...
  return (
    <div className={`${styles.inner} ${className}`}>
        ...
        >
          {items.map((item) => (
            <SwiperSlide key={item.id} className={styles.slide}>
              <CourseCard className="select-none" data={item} />
            </SwiperSlide>
          ))}
      ...
      {!loading && items.length === 0 && debouncedSearch.length == 0 && (
        <div className="text-sm">Вы пока не проходите курсы</div>
      )}
      {!loading && items.length === 0 && debouncedSearch.length > 0 && (
