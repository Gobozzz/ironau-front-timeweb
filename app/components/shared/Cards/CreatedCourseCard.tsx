...
export function CreatedCourseCard({
  className = "",
  data,
  setEditId,
  setDeleteId,
  ...
  const [loadingUpdateLessons, setLoadingUpdateLessons] =
    useState<boolean>(false);

  function handlerUpdateLessons() {
    setLoadingUpdateLessons(true);
    api
      .get(`/courses/${data.id}/owner`)
      .then((response) => {
        setUpdateCourse({
  ...
  return (
    <article className={`${styles.card} ${className}`}>
      ...
      <div className={styles.down}>
        <button
          disabled={loadingUpdateLessons}
          onClick={handlerUpdateLessons}
          className={`${styles.link_edit} ${
            loadingUpdateLessons ? "opacity-50" : ""
          }`}
        >
          Редактировать уроки в курсе
        </button>
        <button
